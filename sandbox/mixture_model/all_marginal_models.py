"""
All 16 Marginal Model Combinations for Normal Mixture

This script generates and runs all 16 combinations of 4 binary model features for the marginal y (no covariates):
1. cluster_specific_mean - Mean varies by cluster (alpha/mu)
...
# =============================================================================
# STAN CODE GENERATOR (MARGINAL)
# =============================================================================

def generate_stan_code_marginal(config):
    """
    Generate Stan model code based on configuration for MARGINAL models (no beta/X).
    
    Parameters:
    -----------
    config : dict with keys:
        - cluster_specific_mean: bool (replaces alpha)
        - spatial_effect: bool
        - cluster_specific_sigma: bool
        - offset: bool
    """
    cluster_mu = config['cluster_specific_mean']
...
""")
    ...
    # Mean (Mu / Alpha)
    if cluster_mu:
        param_lines.append("  vector[K] mu;")
    else:
        param_lines.append("  real mu;")
    ...
    if cluster_mu:
        ...
        model_lines.append("    mu[k] ~ normal(0, 3);")
        model_lines.append("  }")
    else:
        model_lines.append("  mu ~ normal(0, 3);")
...
# =============================================================================
# DATA PREPARATION (MARGINAL - NO COVARIATES)
# =============================================================================

def prepare_data_marginal(data_path, geojson_path, k):
    """Prepare data for marginal model fitting (no X)."""
...
def run_model_marginal(config, config_id, data, output_dir):
    """
    Run a single marginal model configuration.
    """
    print(f"\\n{'='*60}")
    ...
    # Generate Stan code
    stan_code = generate_stan_code_marginal(config)
    
    # Write Stan file
    stan_file = os.path.join(output_dir, f"model_marg_{config_id}.stan")
...
def generate_all_marginal_configs():
    """Generate all 16 marginal model configurations (no beta)."""
    features = ['cluster_specific_mean', 'spatial_effect', 'cluster_specific_sigma', 'offset']
    configs = []
...
def main():
    ...
    # Define output directory for marginal models
    ...
    output_dir = os.path.join(base_dir, 'stan_models_marginal')
    ...
    # Loop through K
    for c in [2, 3]:
        print(f"\\n\\nRunning all marginal models for K={c} clusters")
        
        print("="*70)
        print("ALL 16 MARGINAL MODEL COMBINATIONS")
        print("="*70)
        print(f"Started at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        
        # Prepare data
        print("\\nPreparing data (Marginal - No Regressors)...")
        data = prepare_data_marginal(data_path, geojson_path, c)
        print(f"Data prepared: N={data['N']}, K={data['K']}")
        
        # Generate configurations
        configs = generate_all_marginal_configs()
        ...
        for i, config in enumerate(configs):
            config_id = i + 1
            result = run_model_marginal(config, config_id, data, output_dir)
            results.append(result)
            
            # Save partial
            with open(os.path.join(output_dir, f'results_partial_K{c}.pkl'), 'wb') as f:
                pickle.dump(results, f)
        
        # Save final for this K
        print("\\n" + "="*70)
        print(f"SAVING RESULTS FOR K={c}")
        print("="*70)
        
        with open(os.path.join(output_dir, f'results_K{c}_marginal.pkl'), 'wb') as f:
            pickle.dump(results, f)
        ...
        for r in results:
            row = {
                ...
                'cluster_mean': r['config']['cluster_specific_mean'],
        ...
        summary_csv = os.path.join(output_dir, f'summary_K{c}_marginal.csv')
        ...
        if n_success > 0:
            ...
            print(f"\\nBest model for K={c} (lowest Binder loss):")
        ...
        print(f"\\nFinished K={c} at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
